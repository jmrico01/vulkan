...
const float32 BACKGROUND_DEPTH = 0.05f;
const float32 PANEL_ITEM_DEPTH = 0.0f;

Panel::Panel(LinearAllocator* allocator)
: renderCommands(allocator)
{
}
...
bool Panel::SliderFloat(float32* value, float32 min, float32 max, Vec4 color, const VulkanFontFace* fontFace)
{
	DEBUG_ASSERT(value != nullptr);
    if (flags & PanelFlag::MINIMIZED) {
        return false;
...
bool Panel::InputText(InputString* inputString, bool* focused, Vec4 color, const VulkanFontFace* fontFace)
{
	...
	const VulkanFontFace* fontToUse = fontFace == nullptr ? fontFaceDefault : fontFace;
    Vec2Int textSize = { (int)GetTextWidth(*fontToUse, inputString->ToArray()), (int)fontToUse->height };
    Vec2Int boxSize = { (int)(textSize.x * MARGIN_FRACTION), (int)(textSize.y * MARGIN_FRACTION) };
    ...
    for (uint32 i = 0; i < renderCommands.size; i++) {
        ...
        switch (command.type) {
            ...
            case PanelRenderCommandType::TEXT: {
                PushText(*command.commandText.fontFace, command.commandText.text, command.position, PANEL_ITEM_DEPTH,
